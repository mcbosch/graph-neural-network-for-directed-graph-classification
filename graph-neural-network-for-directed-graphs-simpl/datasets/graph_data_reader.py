...
class DataReader():
    ...
    def help(self):
        '''
        This function is for a deep understanding of the code and debbuging.
        '''
        print('This functions has 4 attributes')
        s = '''
            Arguments \n 
                data_dir -> Folder with txt files\n
                rnd_state -> Defines a random seed if it's asked\n
                use_cont_node_attr ->  Use or not additional float valued node attributes available in some datasets\n
                folds -> devides de dataset in X folds so it trains 1 different fold in each training
            '''
        print(s)
        print('''With this arguments we define 4 attributes for this class: \n
              data_dir, rnd_state, use_cont_atrr. For this case we have:
              ''')
        print(f'data_dir: {self.data_dir}')
        print(f'rnd_state: {self.rnd_state}')
        print(f'use_cont_node_attr: {self.use_cont_node_attr}')

        print(f'''Per l'atribut *data* tenim un diccionari per accedir a diferents\n objectes deñs datasets:\n
                > keys: {self.data.keys()}\n
                Emprarem el DataReader per treballar les dades amb un objecte gdata.
                ''')
    '''
...
class GraphData(torch.utils.data.Dataset):
    ...
    def ad2MagL(Adj, q, normalized):
        ...
        As = 0.5*(Adj + Adj.T)
        D = [sum(As[i]) for i in range(len(As))]
        D_norm = np.diag([np.power(D[i] if D[i] != 0 else 0, -0.5) for i in range(len(D))])
        T = 2*math.pi*q*(Adj - Adj.T)
        T = np.cos(T) + np.sin(T)*1.0j
        I =np.eye(len(Adj)) 

        if normalized:
            return  I - (np.matmul(np.matmul(D_norm, As),D_norm))*T
        else:
            return  I - (np.matmul(np.matmul(D_norm, As),D_norm))*T
    ...
    def help(self):
        '''
        This function is for a debbuging, may help to understant the code and how
        we are working our data
        '''
        print(f'''
              Aquest objecte és un GraphData. Per definir-lo necessitam dels següents\n
              aarguments:\n
              > fold_id: En quin fold ens trobam (depenguent del fold un conjunt o in altre és el que \n 
              empram per entrenar o no)\n
              > datareader: Un  objecte DataReader, el nombre de folds d'aquest ha de ser compatible \n
              amb el fold_id
              > split: defineix si ens trobam en training o test.

              En el nostre cas paritcular tenim:

                self.total = len(data['targets']) {self.total}\n
                self.N_nodes_max: {self.N_nodes_max}\n
                self.max_edge_matrix: {self.max_edge_matrix}\n
                self.n_classes: {self.n_classes}\n
                self.features_dim:{self.features_dim} \n
                self.idx = data['splits'][fold_id][split]: llista de indexos de elements que estan en el conjunt\n
                
            Per definir els següents atributs s'empra deepcopy per evitar problematiques \n
            d'edició de llistes. \n
                (mostram la primera, és una llista de {len(self.idx)} elements)\n
                self.labels: {set(self.labels)}\n
                self.adj_list: {self.adj_list[0]}\n 
                self.features_onehot: {self.features_onehot[0]}\n
                self.max_neighbor_list: {self.max_neighbor_list[0]}\n
                self.edge_matrix_list: {self.edge_matrix_list[0]}\n
                self.node_count_list: {self.node_count_list[0]}\n
                self.edge_matrix_count_list: {self.edge_matrix_count_list[0]}\n
